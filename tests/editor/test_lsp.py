...
from veripy.editor.server import (
    _CONTENT_MODIFIED,
    _MAX_PROVERS,
    Server,
    analyze,
...
def test_tokenizer_error_is_a_diagnostic_not_a_dead_server(monkeypatch):
    ...
    import veripy.editor.server as lsp_mod
...
def test_verify_time_limit_is_clamped_not_trusted(monkeypatch):
    ...
    monkeypatch.setattr("veripy.editor.server.prove", fake_prove)
...
def test_shutdown_waits_for_an_in_flight_proof(monkeypatch):
    ...
    monkeypatch.setattr("veripy.editor.server.prove", slow_prove)
    replies = _run([
        {"jsonrpc": "2.0", "method": "textDocument/didOpen",
         "params": {"textDocument": {"uri": "file:///m.py", "text": GOOD}}},
...
def test_a_genuinely_absent_sidecar_still_proves_normally(tmp_path, monkeypatch):
    ...
    src = tmp_path / "m.py"
    src.write_text(GOOD)
    monkeypatch.setattr("veripy.verification.runner.verify_structured",
...
def test_the_server_refuses_rather_than_queueing_unboundedly(monkeypatch):
    ...
    from veripy.editor.server import _MAX_INFLIGHT, _REQUEST_FAILED
...
def test_an_edit_cannot_land_between_the_check_and_the_reply(monkeypatch):
    ...
    order = []
    uri = "file:///m.py"
    monkeypatch.setattr("veripy.editor.server.prove",
