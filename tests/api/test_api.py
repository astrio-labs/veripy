...
from veripy.verification.failures import is_known
...
def test_root_does_not_shadow_submodules_with_api_names():
    # Internal modules must not overwrite the stable functions in veripy.api.
    import veripy.proofs.repair as repair_module
    from veripy import proofs

    assert proofs.repair is repair_module
    assert not hasattr(veripy, "repair")
