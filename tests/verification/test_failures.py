...
from veripy.verification.failures import (
    FAILURE_KINDS,
    PROVER_KINDS,
    TAXONOMY_VERSION,
    describe,
...
PACKAGE = Path(__file__).resolve().parents[2] / "veripy"
...
def test_prover_and_frontend_kinds_are_disjoint():
    ...
    from veripy.verification.failures import (FRONTEND_KINDS, HARNESS_KINDS,
                                  UNCLASSIFIED_KINDS)
...
def test_unattributable_failure_reports_null_region(tmp_path, monkeypatch):
    ...
    import veripy.verification.runner as agentio_mod
