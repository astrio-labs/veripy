...
from veripy.proofs.repair import (
    RULES,
    _strip_fences,
    build_request,
    make_engine,
...
def test_lean_repair_rejects_axiom_then_checks_the_replacement(tmp_path):
    source = tmp_path / "component.py"
    source.write_text(NEEDS_LEMMA)
    dafny_sidecar = source.with_suffix(".proofs.dfy")
    dafny_sidecar.write_text(GOOD)
    checked_proof = "theorem Obvious (x : Int) : x = x := by rfl\n"
    proposals = iter(["axiom Obvious (x : Int) : x = x\n", checked_proof])
    requests = []
    ...
    outcome = repair_file(source, tmp_path / "repair", engine,
                          backend="lean", max_iterations=2, time_limit=30)
    assert outcome.verified, outcome
    assert outcome.iterations == 2
    assert len(requests) == 2
    assert all(".proofs.lean" in request["rules"] for request in requests)
    assert outcome.attempts[0]["rejection"] is not None
    assert outcome.sidecar_text == checked_proof
    assert source.read_text() == NEEDS_LEMMA
    assert dafny_sidecar.read_text() == GOOD
    assert not source.with_suffix(".proofs.lean").exists()
...
def test_claude_engine_denies_tools():
    ...
    # deny all tools.
    from veripy.proofs.repair import _claude_cmd
...
def test_claude_engine_runs_in_empty_sandbox(monkeypatch):
    ...
    import veripy.proofs.repair as repair_mod
...
def test_engine_wall_is_configurable_and_used(monkeypatch):
    ...
    # answer in time".
    import veripy.proofs.repair as repair_mod
    from veripy.proofs.repair import DEFAULT_ENGINE_WALL_S, _ClaudeEngine
...
def test_non_positive_engine_wall_is_rejected_not_silently_defaulted(capsys, tmp_path):
    ...
    # silently became 600s. A repair that reports its wall must have run under
...
def test_make_engine_model_specs():
    from veripy.proofs.repair import _ApiEngine, _ClaudeEngine
...
def test_parse_claude_json():
    from veripy.proofs.repair import _parse_claude_json
...
def test_claude_engine_records_usage(monkeypatch):
    import veripy.proofs.repair as repair_mod
    from veripy.proofs.repair import _ClaudeEngine
...
def test_cursor_cmd_is_locked_down():
    ...
    from veripy.proofs.repair import _cursor_cmd
...
def test_cursor_engine_sandbox_config_and_live_sample_parse(monkeypatch):
    ...
    import veripy.proofs.repair as repair_mod
    from veripy.proofs.repair import _CursorEngine
...
def test_cursor_parse_survives_non_object_nested_fields():
    ...
    from veripy.proofs.repair import _parse_cursor_json
...
def test_history_digests_proposals_and_reports_drops():
    ...
    from veripy.proofs.repair import history_for_prompt
    ...
    from veripy.proofs.repair import _history_json
...
def test_prompt_states_omissions_and_stays_bounded():
    from veripy.proofs.repair import _render_prompt, build_request
...
def test_prompt_explains_the_kinds_actually_present():
    ...
    # had not typechecked.
    from veripy.verification.failures import FAILURE_KINDS
    from veripy.proofs.repair import _render_prompt, build_request
...
def test_render_prompt_contains_the_repair_source_and_reply_format():
    from veripy.proofs.repair import _render_prompt
...
def test_apply_first_wins_when_sidecar_changed_mid_repair(tmp_path):
    ...
    from veripy.proofs.repair import _apply_sidecar
...
def test_model_substitution_is_refused_not_recorded():
    ...
    """
    from veripy.proofs.repair import _ClaudeEngine, _parse_claude_json
...
def test_engine_effort_reaches_the_cli_and_is_validated(monkeypatch):
    """Reject invalid effort levels and pass valid levels to the engine."""
    import veripy.proofs.repair as repair_mod
    from veripy.proofs.repair import _claude_cmd, _ClaudeEngine
