...
predicate LuhnCompleted(s: string, a: string, digit: string)
  requires a == "0123456789"
  requires forall c :: c in s ==> PyStrFind(a,[c]) >= 0
{
  |digit| == 1 && (forall c :: c in digit ==> PyStrFind(a,[c]) >= 0) &&
  PyMod(LuhnTotal(LuhnDigits(s+digit,a),10),10) == 0
}
lemma LuhnIndexSearch(a: string, j: int, k: int)
  requires 0 <= k <= j < |a|
  requires forall i :: 0 <= i < j ==> a[i] != a[j]
  ensures PyStrFind(a[k..],[a[j]]) == j-k
  decreases j-k
{
  assert a[k..][..1] == [a[k]];
  if k < j {
    LuhnIndexSearch(a,j,k+1);
    assert a[k..][1..] == a[k+1..];
  }
}
lemma LuhnDigitIndex(a: string, j: int)
  requires a == "0123456789"
  requires 0 <= j < 10
  ensures PyStrFind(a,[a[j]]) == j
{
  assert forall i :: 0 <= i < j ==> a[i] != a[j];
  LuhnIndexSearch(a,j,0);
}
lemma LuhnHead(v: seq<int>, d: int)
  ensures LuhnTotal([d]+v,10) == d + LuhnTotal([0]+v,10)
{
  if |v| > 0 { assert ([d]+v)[2..] == ([0]+v)[2..]; }
}
lemma CheckDigitComplete(s: string, a: string, ck: int)
  requires a == "0123456789"
  requires forall c :: c in s ==> PyStrFind(a,[c]) >= 0
  requires 0 <= ck < 10
  requires LuhnExact(s+[a[0]],a,ck)
  ensures LuhnCompleted(s,a,[a[if ck == 0 then 0 else 10-ck]])
{
  var j := if ck == 0 then 0 else 10-ck;
  LuhnDigitIndex(a,0);
  LuhnDigitIndex(a,j);
  var v := LuhnDigits(s,a);
  assert (s+[a[0]])[..|s|] == s;
  assert (s+[a[j]])[..|s|] == s;
  assert LuhnDigits(s+[a[0]],a) == [0]+v;
  assert LuhnDigits(s+[a[j]],a) == [j]+v;
  LuhnHead(v,j);
  var total := LuhnTotal([0]+v,10);
  assert ck == PyMod(total,10);
  assert PyMod(total+j,10) == 0;
  assert LuhnExact(s+[a[j]],a,0);
}
