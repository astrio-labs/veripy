#@ ensures True
def page_aligned_decode_alloc_lens(reqs: list[RequestSnapshot], *, reserve: int, page_size: int) -> tuple[list[int], list[int], int]:
    ...
    for i, r in enumerate(reqs):
        ...
        nxt = max(cur, (r.kv.kv_committed_len + reserve + page_size - 1) // page_size * page_size)
        cur_kv_lens[i] = cur
        nxt_kv_lens[i] = nxt
