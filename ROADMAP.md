...
- [ ] `#@` spec grammar v0: `requires` / `ensures` / `invariant` / `decreases` / `mutates` / `extern`, ghost `forall` / `exists` / `old()` / `result`
- [ ] Spec parser: parse, name-resolve against basedpyright, type-check spec expressions
- [ ] Compile the same specs to CrossHair / icontract runtime checks; run on real dynamic Python for counterexample-producing results now
