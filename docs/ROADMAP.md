...
| triple adjudication (benchmark corpus, 16 tasks) | **15 proved under EVERY backend, 0 conclusive splits, 0 unadjudicated** (the day's arc: 8 with 1 split at first measurement → the split resolved by the square-maximality endgame, five gaps by the pack slice, `is_prime` by the sqrt-search class). The ONE named gap: `sum_to_n` — the assert-vs-pack coupling above. Re-run: `veripy benchmark --backend all` |
