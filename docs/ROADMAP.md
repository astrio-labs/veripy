...
| triple adjudication (benchmark corpus, 16 tasks) | **14 proved under EVERY backend, 0 conclusive splits, 0 unadjudicated** (was 8 with 1 split at first measurement — the split resolved by the square-maximality endgame, five gaps by the pack slice). The 2 named gaps: `is_prime` (early `return` in a `while` body — no while twin of the for-path search desugar) and `sum_to_n` (the assert-vs-pack coupling above). Re-run: `veripy benchmark --backend all` |
...
| `is_prime` under Lean | outside the fragment: an early `return` inside a `while` body (the for-path search desugar has no while twin) — the named blocker for the last sidecar-gap task |
