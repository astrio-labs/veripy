...
- [ ] Fragment IR (thin) + Dafny encoder: two-file output (regenerated stub + additions-only proof file) — **slice 1 shipped**: clean-bucket encoder (`lemmapy verify`), single self-contained stub with a STUB END marker (additions-only enforced diff-wise, benchmark-style); 6 functions verified end-to-end incl. loop invariants; seeded clamp bug caught statically with the failure mapped to the Python line. **Slice 3 shipped**: list building — `append` under ownership-lite (§3.2 conservative: fresh, unaliased, not-under-iteration), list literals, filterless comprehensions → `seq(n, i => ...)`, for-each over lists (snapshot + hidden index), §7.3 list truthiness in conditions; `incr_list` and `intersperse` verified (8 functions total). **Slice 4 shipped**: `Optional[T]`/`T | None` → `PyOpt<T>` with narrowing replayed as `.v` well-formedness VCs, `is [not] None`, Some-injection/projection coercions, Python-exact `==`-with-None semantics; slices → clamped `PySlice`; 1-arg `max`/`min` → `PySeqMax`/`PySeqMin` (empty-seq requires = Python's ValueError); Python `assert` lowers to Dafny assert (runtime check + proof hint — the X-ASSERT candidate graduates); index policy: provably-nonneg indices bare for trigger compatibility, PyIndex otherwise; `rolling_max` verified (9 functions total; preamble 0.3). **Slice 6 shipped**: `sum()` folds — `sum(<list[int]>)` and filterless `sum(<genexp over a list>)` → `PySum` (preamble 0.4, snoc-recursive so the established executable-`assert` slice-extension idiom steps running-sum invariants with no new lemma machinery); keyword arguments to encoded builtins now rejected (previously silently dropped — `max(a, b, key=abs)` miscompiled); `below_zero` (HumanEval/3) and `sum_squares` verified (12 functions total, benchmark 12/12 at full ladder, 42/42 mutants killed). The slice's adversarial round closed two verified-but-false name-resolution holes that predate it: any module/parameter/local binding that shadows an encoder-builtin (`len`/`min`/`max`/`abs`/`sum`/`range`/`bool`/`all`/`any`/`old`) is rejected (an unspecced `def sum` used to vanish from the model while call sites encoded as the builtin), and quantifier/comprehension binders that collide with an enclosing binder are rejected (name_overrides silently rewrote the bound variable); `range(...)` keywords rejected at every structural matcher (CPython TypeError, previously read as positional-only). **Slice 7 shipped**: `**` → `PyPow` (preamble 0.5; requires e >= 0 — Python yields a float for negative exponents, so the requires is exactly the int fragment's domain condition); `modp` (HumanEval/49) verified with a 6-lemma mod/pow sidecar grounded like the gcd pack, and `triples_sum_to_zero` (HumanEval/40) verified as-is (14 corpus functions total; exam roster n=2)
- [ ] Versioned Dafny preamble: `PyMod` / `PyFloorDiv`, `PyIndex` / `PySlice`, `Truthy_*`, `Option`, container/str models — with lemmas and its own differential test corpus. **v0.4 shipped** (`PyMod`/`PyFloorDiv`/`PyMin`/`PyMax`/`PyAbs`/`PyIndex`/`PySlice`/`PyOpt`/`PySeqMax`/`PySeqMin`/`PySum`). **The divisibility lemma pack shipped** as gcd's proof sidecar (`he_humaneval_13.proofs.dfy`, 8 ghost lemmas grounded in an inductive `MulPositive`) invoked via the new `#@ proof` clause — the designated first proof-additions case, done: gcd VERIFIED with its full maximality spec (10 functions total). ~~Promote the pack to the preamble when a second user appears~~ *Decision (Aug 2026): a second consumer exists (`is_prime` reuses the divisibility family), but promotion is deliberately **deferred** — the proof-repair exam measures restoration of exactly these lemmas, so a preamble copy would make every sidecar vacuous (pinned by `test_sidecar_is_load_bearing`). Revisit once the exam corpus is decoupled from the preamble version.*
- [x] Guard generator: deep exact-type checks, executable preconditions, copy-in, trusted-caller elision, blame errors (§4) — `lemmapy guard` emits `<stem>_guarded.py`: a **verbatim island copy** of the verified source plus boundary wrappers with descriptor-driven deep exact-type checks (`type(x) is int` exactness — `json.loads` bools/floats, `typing.cast` lies, and `EvilList` subclasses all rejected with the offending value path), depth-complete copy-in, executable `requires` (blame: caller), optional `--check-ensures` incl. `old()` snapshots (blame: callee-or-toolchain); because the island is a copy, `mock.patch` on the original module cannot reach what the boundary runs; trusted callers elide by importing the original. The §1 attack gallery is demonstrably stopped (tests/test_guards.py)
- [x] Island integrity hardening + assumptions A1–A7 in the verification report (§5) — guarded wrappers bind the island and helpers in closure cells at definition time (module-attribute rebinding cannot redirect them; cell surgery stays under A1), the island region carries an embedded SHA-256 with `verify_island_integrity()` for on-disk tamper detection, and A1–A7 ship verbatim in the report with per-assumption discharge status
- [x] Verifier driver: failure mapping back to Python source ✅ (slice 1); verification report shipped — `lemmapy verify --report out.json`: per-function verdicts with failures attributed by source span, spec surface (every requires executable, so the assumed-clause set is empty by construction), sidecar lemma inventory, guard mode per entry point, "verified modulo 0 trusted contracts" (computed), A1–A7 with discharge status
- [x] Translation-validation harness (§6): `lemmapy difftest` — `dafny translate py` + Hypothesis over typed signatures filtered by executable `#@ requires`, compared across the value adapter; runs in the test suite over the slice-1 targets (incl. unproven gcd — fidelity is orthogonal to proof); GitHub Actions runs the harness on every PR (unit tier without Dafny + full prove tier with Dafny and the quick benchmark scorecard). Nightly coverage-guided long runs pending
- [x] Fragment semantics note: big-step semantics for the fragment + simulation statement (paper, not mechanized) — [SEMANTICS.md](SEMANTICS.md): value sorts, judgment forms with modeled-error outcomes, the divergence-prone rules (floor div/mod, indexing, slicing, folds, quantifier scoping, truthiness), the simulation claim (a)–(c) with proof sketch, and the per-construct differential-coverage table
...
- [x] LLM proof-repair loop: read failures, edit proof file only, re-verify, iterate; M0 counterexamples fed in as test cases — `lemmapy repair`: the engine (headless `claude -p` by default; `file:<dir>` scripted engine for tests/replays) receives the structured-failure payload + frozen source + current sidecar and may propose ONLY new sidecar content; proposals pass through the same whitelist validator as hand-written sidecars (a repair can never smuggle an axiom) and rejections come back as the next iteration's feedback; all work on copies, `--apply` writes back on success with a `.bak`; counterexamples attach via `verify --json --hunt-counterexamples`
- [x] Tune the loop against **benchmark-derived proof-repair exams** ([BENCHMARK.md](BENCHMARK.md)): strip the proof additions from golden tasks and score R4-restoration under frozen specs — our own corpus, our own harness, no competitor infrastructure. Shipped: `lemmapy benchmark --exam proof-repair` — sidecars stripped, `#@ proof` clauses stay in the frozen source, restoration re-earned through the whitelist + prover; the gcd exam (8-lemma pack) restores end-to-end in the test suite via a scripted golden engine, and `--engine claude` runs the live loop
...
**Exit criteria:** measured LLM proof-completion rate on the M1 benchmark suite without human edits (the headline DX metric) — **measured (August 2026): 1/2 proof-repair exams restored at the default budget on the n=2 roster — gcd twice (once via a 5-lemma pack smaller than the golden 8), modp's nonlinear congruence not within budget; [EVALUATION.md](EVALUATION.md) has the numbers, the pinned invocation, and the two invalid runs that preceded them**; a newcomer can take an unannotated fragment-conformant function to "verified" using only tool feedback.
