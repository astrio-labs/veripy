...
| exams under `--backend lean` | roster follows the backend's sidecars; the 4 contact packs screen **load-bearing**; the he_31 exam restores end to end with the scripted golden. The benchmark corpus now carries 5 `.proofs.lean` twins: `gcd`/`modp` (ports of the he_13/he_49 packs, load-bearing) and `below_zero`/`sum_squares`/`rolling_max` (resolution-only — the prelude carries their content, and the per-backend screen marks them vacuous, keeping them off the Lean exam roster while resolving the shared `#@ proof` clauses) |
| Lean proof-repair exam, FIRST REAL ENGINE (2026-08-29) | default `claude` engine, default effort, prover 60s, engine wall 600s. **4-iteration cell (full 7-row roster): restored 3/7** — all three resolution-only rows in 1–2 iterations (the engine authors whitelist-clean, kernel-checked packs); **load-bearing 0/4** — gcd/is_prime/sum_to_n exhausted the budget on REAL proofs failing core-Lean surface details (gcd's final attempt: an fmod/emod bridge over `Int.fdiv_add_fmod` with a Euclid congruence helper), modp wrote a Nat-typed pack against Int goals then hit the wall. **8-iteration cell (load-bearing four): 0/4** — every task pushed past the old ceiling (iterations 4–7) and then hit the ENGINE WALL: raising the budget converts iteration exhaustion into wall exhaustion, the same thinking-time bind the Dafny runs measured. The instrument DISCRIMINATES: pack validity (3/3) vs load-bearing proof under core-Lean constraints (0/4). All attempts preserved per row. Harness follow-up noted: a guarded clause-instantiation `have` leaves `sorry ()` elaboration debris in DIAGNOSTIC hypotheses when a mistyped pack lemma meets its argument — harmless (the proof fails honestly and the axiom gate would refuse sorryAx), but it should decline cleanly |
| Lean spec-writing exam, FIRST REAL ENGINE (2026-08-29) | default `claude` engine, quick-mode panel (mutant cap 3 — a SCOUT measurement, recorded as such), full 16-task roster, Lean-backed ladder. **Validity 16/16** — every task got a mechanically valid, source-frozen spec (one mid-run streak of 11 empty engine failures was a transient CLI outage, re-measured cleanly against the cached golden baselines). **Spec strength: engine 30/42 mutants refuted (71%) vs golden 32/42 (76%)** on identical panels; timeout-bias check passed both passes, so the 2-mutant gap is a genuine strength gap. 9 mutants crashed under engine specs (interpreter-caught, never credited). Engine specs MATCHED golden refutation exactly on 10 of 16 rows; the gap concentrates in `modp` (0/3 vs 2/3) and crash-heavy rows (`intersperse`, `rolling_max`, `sum_squares` — 2 crashes each). Full-panel (cap 12) run is the named follow-up before quoting outside. Re-run: `veripy benchmark --exam spec-writing --backend lean --quick` |
| the sum_to_n coupling (measured) | adding the exit-value assert lets the Lean ladder prove it — and flips the DAFNY pack vacuous (Z3 no longer needs `GaussStep`), which would cost a proof-repair exam row. The exam outranks the Lean win: the task stays Dafny-only until the Lean while-endgame derives exit values unaided |
| `is_prime` under Lean | **resolved by the sqrt-search class**: the while trial division is a pinned (Int × Bool) fold, and the sidecar's `CompositeHasSmallFactor` is applied at the True exit with every side condition discharged — the first Lean exam row load-bearing by construction |
...
| `dict` | **RESOLVED, both tasks.** `mbpp_97` by the frequency-dict class (lean-0.16). `mbpp_885` by the isomorphism class (lean-0.17): the "canonicalization argument several times any pack" collapsed to ordinary size on two design facts — first-occurrence order is a property of the position PARTITION alone, so pattern-equality gives literally equal value lists (`IsoVals`), and the sort needs no properties beyond being a permutation (`SortL_perm`); the spec theorem itself proves the multiset test equivalent to the ∀∀ pattern, which is also what makes the opaque sort model faithful |
