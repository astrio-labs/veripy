...
Admitted surface today (everything else is a detected, explained rejection): `int` / `bool` / `str` / `list[T]` / `Optional[T]`; assignment, `if`/`elif`/`else`, `while`, `for i in range(...)`, `for x in xs`, `break`/`continue` (continue on a desugared `for` steps the hidden index first), `return`; arithmetic via `PyFloorDiv`/`PyMod`/`PyPow`; `append` on owned fresh lists; specs `requires`/`ensures`/`invariant`/`decreases`/`proof`. See the encoder module docstring for the miscompilation classes it closes.
