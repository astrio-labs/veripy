...
def _import_target(expr, declarations):
    """Resolve only a unique, absolute import, never an executable alias."""
    if isinstance(expr, ast.Attribute) and isinstance(expr.value, ast.Name):
        bindings = declarations.get(expr.value.id, [])
        if len(bindings) == 1 and isinstance(bindings[0], ast.Import) and bindings[0].lineno < expr.lineno:
            for alias in bindings[0].names:
                if (alias.asname or alias.name.split('.')[0]) == expr.value.id:
                    if alias.name in {'typing', 'typing_extensions', 're'}:
                        return alias.name, expr.attr
    if isinstance(expr, ast.Name):
        bindings = declarations.get(expr.id, [])
        if len(bindings) == 1 and isinstance(bindings[0], ast.ImportFrom) and bindings[0].lineno < expr.lineno:
            node = bindings[0]
            if not node.level and node.module in {'typing', 'typing_extensions', 're'}:
...
def _overload_group(bindings, declarations):
    # Keep all overload signatures in the candidate. Resolving the implementation
    # must not erase type information or bless arbitrary same-name redefinitions.
    if len(bindings) < 2 or not all(isinstance(n, ast.FunctionDef) for n in bindings):
        return False
    if bindings[-1].decorator_list:
        return False
    for n in bindings[:-1]:
        if len(n.decorator_list) != 1 or _import_target(n.decorator_list[0], declarations) not in {
...
def _inert_namespace_call(call, declarations):
    """An omitted declaration may initialize literal regex/type metadata.

    This is only a noninterference rule for a declaration view, not a regex
    semantics model. Retained calls still go through backend admission. No
    arbitrary expressions, callbacks, dynamic aliases or imports are evaluated.
    """
    target = _import_target(call.func, declarations)
    if target == ('re', 'compile'):
        if not 1 <= len(call.args) <= 2 or not isinstance(call.args[0], ast.Constant) or type(call.args[0].value) not in (str, bytes):
            return False
        if any(k.arg != 'flags' for k in call.keywords) or len(call.keywords) > 1 or (len(call.args) == 2 and call.keywords):
            return False
        def flag(n):
            return (isinstance(n, ast.Constant) and type(n.value) is int and n.value == 0 or
                    _import_target(n, declarations) in {('re', f) for f in ('A','ASCII','I','IGNORECASE','M','MULTILINE','S','DOTALL','X','VERBOSE','U','UNICODE','L','LOCALE','NOFLAG')} or
                    isinstance(n, ast.BinOp) and isinstance(n.op, ast.BitOr) and flag(n.left) and flag(n.right))
        return all(flag(n) for n in call.args[1:]) and all(flag(k.value) for k in call.keywords)
    if target in {('typing', 'TypeVar'), ('typing_extensions', 'TypeVar')}:
        return (len(call.args) == 1 and isinstance(call.args[0], ast.Constant) and type(call.args[0].value) is str and
                len({k.arg for k in call.keywords}) == len(call.keywords) and
                all(k.arg in {'bound', 'covariant', 'contravariant'} and isinstance(k.value, ast.Constant) and
                    (type(k.value.value) is str if k.arg == 'bound' else type(k.value.value) is bool) for k in call.keywords))
    return False
...
def extract_candidate(source: str, entry: str) -> dict:
    ...
    selected={};edges=[];nodes={};pending=[entry];seen=set();preparation_rules=[]
    overloads={name: bindings for name, bindings in declarations.items() if _overload_group(bindings, declarations)}
    while pending:
        name=pending.pop()
        if name in seen:continue
        seen.add(name)
        bindings=declarations.get(name,[])
        if len(bindings)>1 and name not in overloads:
        ...
        if not bindings:
            ...
            continue
        n=bindings[-1] if name in overloads else bindings[0]
        ...
        retained=bindings if name in overloads else [n]
        for declaration in retained:selected[declaration.lineno]=declaration
        if name in overloads:
            nodes[name]['overload_lines']=[b.lineno for b in bindings[:-1]]
            preparation_rules.append({'kind':'typing-overload-group','name':name,'implementation_line':n.lineno,'retained_signatures':[b.lineno for b in bindings[:-1]]})
        if kind!='import':
            refs=set().union(*(_references(b) for b in retained))
    ...
    if entry not in declarations or (len(declarations[entry])!=1 and entry not in overloads) or not isinstance(declarations[entry][-1],ast.FunctionDef):
        issues.append({'kind':'entry-must-be-unique-function','name':entry})
    watched_modules={item['module'] for (_,name),item in imports.items() if name in seen and item['module'] in {'typing','typing_extensions','re'}}
    watched=seen | {name for (_,name),item in imports.items() if item['module'] in watched_modules}
    ...
    for n in tree.body:
        ...
        for x in ast.walk(n):
            ...
            if isinstance(x,ast.Call) and isinstance(x.func,ast.Attribute):
                base=x.func.value
                if isinstance(base,ast.Name) and base.id in watched and n.lineno not in selected:
                    # Only omit simple declarations whose entire call is in the
                    # reviewed literal metadata grammar. Nested calls still fail.
                    if isinstance(n,(ast.Assign,ast.AnnAssign)) and _inert_namespace_call(x,declarations):
                        preparation_rules.append({'kind':'omitted-literal-metadata-call','name':base.id,'line':n.lineno,'operation':'.'.join(_import_target(x.func,declarations))})
                    else:
