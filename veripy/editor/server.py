...
`veripy lsp` speaks Language Server Protocol over stdio at two speeds,
...
- **instant, automatic** — line-precise diagnostics on open/change/save
  ...
  the custom request `veripy/functionStatus`.
- **slow, explicit** — proof status from the real prover, via the custom
  request `veripy/verify`. Never on the keystroke path: a client binds it
...
older `veripy/verify` still in flight is superseded — answered with
...
from veripy.frontend.parse import ModuleSpecs
...
# `veripy/verify` far faster than the prover answers, and each request used
...
def prove(text: str, filename: str, time_limit: int = 20) -> dict[str, Any]:
    ...
    """
    from veripy.verification.runner import new_payload, verify_structured
...
class Server:
    def __init__(self, reader: BinaryIO, writer: BinaryIO):
        ...
        # sequence number of the newest `veripy/verify` for that document,
    ...
    def _invalidate(self, uri: str) -> None:
        ...
        matched) but the REPLY reached the client anyway, so `veripy/verify`
    ...
    def handle(self, msg: dict[str, Any]) -> None:
        ...
        elif method == "veripy/functionStatus":
            uri = params["textDocument"]["uri"]
            text = self.documents.get(uri, "")
            filename = unquote(urlparse(uri).path) or uri
