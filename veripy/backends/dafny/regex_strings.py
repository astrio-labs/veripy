"""Closed language models for pinned Packaging and Werkzeug regex patterns.

Name matching, separator substitution, and the explicitly registered HTTP
captures are modeled. Arbitrary regexes and capture APIs remain unsupported.
Preserve Python's terminal-newline anchor rule and upstream lookahead placement.
"""
import ast
import re

from veripy.backends.dafny import http_strings, header_charsets

PATTERNS = {
    header_charsets.PATTERN: ("charset",False,header_charsets.FLAGS),
    ...
    r'^([A-Z0-9]|[A-Z0-9][A-Z0-9._-]*[A-Z0-9])\Z': ('validate',True,2),
...
def declarations(module):
    ...
    for node in module.body:
        ...
        if (isinstance(node,ast.Assign) and len(node.targets)==1 and isinstance(node.targets[0],ast.Name)
            ...
            flags=0
            if len(call.args)==2:
                def literal_flags(flag):
                    if isinstance(flag,ast.Attribute) and isinstance(flag.value,ast.Name) and flag.value.id in aliases:
                        return {'IGNORECASE':2,'ASCII':256,'VERBOSE':64}.get(flag.attr)
                    if isinstance(flag,ast.BinOp) and isinstance(flag.op,ast.BitOr):
                        left,right=literal_flags(flag.left),literal_flags(flag.right)
                        return None if left is None or right is None else left|right
                    return None
                flags=literal_flags(call.args[1])
                if flags is None:remaining.append(node);continue
            ...
            if pattern.endswith(r'\Z'):bindings[name]['strict_end']=True
...
def call_model(encoder,node):
    charset=header_charsets.call_model(encoder,node)
    if charset is not None:return charset
    ...
    if f.attr in {'match','fullmatch'} and model['kind'] in {'validate','normalized'}:
        ...
        if f.attr=='match' and not model['anchored']:
            ...
            if f.attr=='match' and not model.get('strict_end',False):value=f'VRegexAnchorText({value})'
...
def guard_regex(function,expected):
    ...
    def checked(*args,**kwargs):
        ...
        for name,model in expected.items():
            ...
            if type(actual) is not pattern_type or actual.pattern!=model['pattern'] or actual.flags!=(model['flags']|(0 if model['flags']&256 else 32)):
...
# All shared model declaration names are reserved against user shadowing.
from veripy.backends.dafny.http_lists import NAMES as HTTP_LIST_NAMES
NAMES = NAMES | HTTP_LIST_NAMES

PREAMBLE += header_charsets.PREAMBLE
NAMES = NAMES | header_charsets.NAMES
