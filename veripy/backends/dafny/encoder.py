...
from ...frontend.records import RecordError, record_schemas, record_type, record_constructor, record_field
from .preamble import PREAMBLE, PREAMBLE_NAMES
from . import unicode_strings, regex_strings, checksum_sequences
PREAMBLE_NAMES = PREAMBLE_NAMES | unicode_strings.NAMES | regex_strings.NAMES
...
class _MethodEncoder:
    ...
    def _build_mangle_map(self) -> dict[str, str]:
        ...
        for name in sorted(self.used_names):
            if name in DAFNY_KEYWORDS or name.startswith("_"):
        ...
        match node:
            ...
            case ast.Name(id=name):
                if name not in self._shadowed and name not in self.types and name not in self.name_overrides and name in self.module_values:
                    from .environment import literal
    ...
    def _compare(self, node: ast.Compare, left: ast.expr, ops, comps) -> str:
        ...
        for op, comp in zip(ops, comps):
            if isinstance(op,(ast.Eq,ast.NotEq,ast.In,ast.NotIn)):
                ...
                if any(token in kind for kind in kinds for token in ('VRegexMatch','VHttpMatch','VHttpETags')):
            ...
            if isinstance(op, (ast.Lt, ast.LtE, ast.Gt, ast.GtE)):
                ...
                if {"PyOpt<VRegexMatch>","PyOpt<VHttpMatch>"} & {lt,rt}:raise _err(node,"regex match objects support presence checks, not value equality")
    ...
    def _call(self, node: ast.Call) -> str:
        ...
        if name in ("all", "any") and len(args) == 1 and isinstance(args[0], ast.Call):
            from ...frontend.literal_map import expand_literal_map
    ...
    def _str_method(self, node: ast.Call, func: ast.Attribute) -> str:
        ...
        if method in ("startswith", "endswith"):
            ...
            if isinstance(args[0], ast.Tuple):
                ...
                parts = [f"{fn}({recv}, {self._require_str_arg(node, method, arg, kind)})" for arg in args[0].elts]
    ...
    def _bool_ctx(self, test: ast.expr) -> str:
        ...
        if isinstance(test, ast.Call):
            ...
            if model is not None and model[0]=="PyOpt<VRegexMatch>":return f"({model[1]}).PySome?"
        if self._infer(test) == "PyOpt<VRegexMatch>":return f"({self.expr(test)}).PySome?"
