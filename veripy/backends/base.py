...
- Registration is lazy: `get_backend` imports a backend's module on first
  use, so importing `veripy` never pays for provers that are not
  installed.
"""
...
class DiagnosticLike(Protocol):
    ...
    (`veripy-failures/1` carries both coordinate systems under that
...
# Lazy so `import veripy` never imports a prover integration it does not
