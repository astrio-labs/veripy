...
from veripy.backends.dafny.encoder import EncodeError, validate_sidecar_text

Engine = Callable[[dict[str, Any]], str]

# Bound each engine call separately from verifier time. An engine timeout
# is an environment outcome, not a proof failure.
...
# its key is present, so callers may omit optional feedback.
...
def history_for_prompt(
    ...
    # Preserve caller-supplied feedback and digest only bulky proposal text.
    kept = []
