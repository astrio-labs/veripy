...
proof status is established separately by `veripy verify`), then redefines each
...
def _reject_reserved_names(module: ast.Module) -> None:
    """Every identifier the generator emits is `_veripy`-prefixed; no name
...
def _signature(fn: ast.FunctionDef) -> tuple[str, str, list[tuple[str, ast.expr | None]]]:
    ...
    from ..backends.dafny.encoder import _checked_defaults, EncodeError
    try:
...
def _wrapper(fn: ast.FunctionDef, spec: FunctionSpec, check_ensures: bool, records=None) -> list[str]:
    ...
    for clause in spec.by_kind("requires"):
        ...
        body.append("    try:")
        body.append(f"        _veripy_ok = bool({clause.desugared})")
        body.append("    except Exception as _veripy_exc:")
        body.append(
            f"        raise _veripy_bound_pre({name!r}, "
            f"{'requires ' + clause.raw!r} + "
            f"f' raised {{type(_veripy_exc).__name__}}: {{_veripy_exc}}')"
        )
        body.append("    if not _veripy_ok:")
        body.append(
            f"        raise _veripy_bound_pre({name!r}, "
            f"{'requires ' + clause.raw!r})"
        )
    ...
    for n in old_names:
        ...
        body.append(f"    _veripy_old_{n} = _veripy_bound_copy({n}, {descs[n]!r})")
    if ensures and params:
        ...
        for part in call_args.split(", "):
            p = part.split("=")[0]
            copied = f"_veripy_bound_copy({p}, {descs[p]!r})"
            island_parts.append(f"{p}={copied}" if "=" in part else copied)
        body.append(f"    result = _veripy_bound_island({', '.join(island_parts)})")
    else:
        body.append(f"    result = _veripy_bound_island({call_args})")
    for clause in ensures:
        expr = rewrite_old(clause.desugared, "_veripy_old_{name}")
        body.append(f"    if not ({expr}):")
        body.append(
            f"        raise _veripy_bound_post({name!r}, "
    ...
    lines = [f"_veripy_island_{name} = {name}", "", ""]
    lines.append(
        f"def _veripy_make_{name}(_veripy_bound_island, _veripy_bound_guard, "
        f"_veripy_bound_copy, _veripy_bound_pre, _veripy_bound_post):"
    )
    ...
    lines.append("")
    lines.append(
        f"{name} = _veripy_make_{name}(_veripy_island_{name}, "
        f"_veripy_guard_value, _veripy_copy_value, "
        f"_VeripyPreconditionError, _VeripyPostconditionError)"
    )
    lines.append("")
    lines.append("")
    return lines
...
def emit_guarded(
    ...
    guarded: list[str] = []
    out: list[str] = [
        f'"""Generated by `veripy guard` from {src_name} -- DO NOT EDIT.',
        "",
        ...
        "by `veripy verify` (the guards enforce the written spec either",
        ...
        "    PostconditionError as _VeripyPostconditionError,",
        "    PreconditionError as _VeripyPreconditionError,",
        "    copy_value as _veripy_copy_value,",
        "    guard_value as _veripy_guard_value,",
        ")",
        "",
        ...
        "",
        f'_VERIPY_ISLAND_SHA256 = "{hashlib.sha256(source.rstrip(chr(10)).encode()).hexdigest()}"',
        "",
        "",
    ]
    if records:
    ...
    from ..backends.dafny.environment import resolve_for_encoder, EnvironmentError
    try:
