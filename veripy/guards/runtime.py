...
_ISLAND_BEGIN = "# ---- VERIPY ISLAND BEGIN (verbatim copy of the admitted source) ----\n"
_ISLAND_END = "\n# ---- VERIPY ISLAND END ----"


def verify_island_integrity(guarded_path) -> str:
    """Recompute the island digest of a guarded module on disk and compare
    it with the embedded `_VERIPY_ISLAND_SHA256`. Returns the digest, or
    ...
    m = re.search(r'^_VERIPY_ISLAND_SHA256 = "([0-9a-f]{64})"$', text, re.M)
    if m is None:
